...
import { clearAddItemDraft, readAddItemDraft } from "@/lib/add-item-draft";
...
export function AddItemFullPage() {
  const router = useRouter();
  const search = useSearchParams();
  const folderFromUrl = search.get("folderId") || "";
  ...
  const [destId, setDestId] = useState(folderFromUrl);
  ...
  useEffect(() => {
    ...
    if (draft) {
      ...
      setMinQuantity(draft.minQuantity);
      setPrice(draft.price);
      setNotes(draft.notes);
      setProductLink(draft.productLink);
      setTags(draft.tags);
      setCustom(draft.custom);
      setVariantsEnabled(draft.variantsEnabled);
    ...
    api<{ tree: TreeFolder[]; rootId: string }>("/api/v1/folders").then((d) => {
      setTree(d.tree);
      setRootId(d.rootId);
      setDestId((current) => current || folderFromUrl || d.rootId);
    });
    api<{ units: { id: string; name: string; isDefault: boolean }[]; fields: CustomFieldDef[] }>(
      "/api/v1/settings/lookups",
    )
      .then((d) => {
        const itemFields = d.fields.filter((f) => f.appliesTo !== "FOLDER");
        setUnits(d.units);
        ...
        setCustom((prev) => ({ ...defaultsFromFields(itemFields), ...prev }));
      })
      .catch(() => null);
  }, [folderFromUrl]);
  ...
  async function submit(mode: "close" | "another") {
    ...
    try {
      validateItemCustomFields(fields, custom);
    } catch (err) {
    ...
    <div className="flex min-h-0 flex-1 flex-col bg-white">
      <div className="min-h-0 flex-1 overflow-y-auto px-8 py-6 scrollbar-thin">
        <div className="mb-4 flex items-center gap-2 text-[15px] text-[#3d4f47]">
          <Package className="h-4 w-4 text-[#9aa6a0]" />
        ...
        <div className="mt-8 grid gap-10 lg:grid-cols-[minmax(0,1fr)_340px]">
          <div className="space-y-8">
            <section>
              <h2 className="mb-4 text-[11px] font-bold uppercase tracking-[0.08em] text-[#8a9a93]">Inventory</h2>
              ...
              <label className="mt-4 flex items-center gap-2 text-sm text-[#4a5c54]">
                <input
                  type="checkbox"
            ...
            <section>
              <h2 className="mb-4 text-[11px] font-bold uppercase tracking-[0.08em] text-[#8a9a93]">Item details</h2>
              ...
              <textarea
                ...
                rows={4}
                className="mb-4 min-h-[96px] w-full resize-y rounded-md border border-[#d8dfdb] px-3 py-2.5 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20"
              />
              <label className="mb-1.5 block text-[13px] text-[#6b7c74]">Product Link</label>
              <input
                value={productLink}
                ...
                {fields.map((field) => (
                  <div key={field.id}>
                    ...
                    <CustomFieldControl
                      ...
                      onChange={(next) => setCustom((c) => ({ ...c, [field.id]: next }))}
                    />
                  </div>
                ))}
              </div>
